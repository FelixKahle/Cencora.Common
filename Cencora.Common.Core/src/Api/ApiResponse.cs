...
namespace Cencora.Common
{
    ...
    public struct ApiResponse<T> : IEquatable<ApiResponse<T>>
    {
        ...
        public ApiResponse<R> Into<R>(Func<T, R> payloadConverter)
        {
            ...
            {
                Trace.TraceWarning("Payload converter is null.");
            }
            ...
            return Match(
                success: (payload, statusCode) =>
                {
                    if (payloadConverter == null)
                    {
                        throw new InvalidOperationException("Cannot convert a successful response without a payload conversion function.");
                    }
