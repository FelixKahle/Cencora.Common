...
namespace Cencora.Common
{
    ...
    public struct ApiResponse : IEquatable<ApiResponse>
    {
        ...
        /// <summary>
        /// Creates a new instance of the <see cref="ApiResponse"/> struct that represents a successful response with a status code of 200.
        /// </summary>
        /// <returns>A new instance of the <see cref="ApiResponse"/> struct that represents a successful response.</returns>
        ...
        public R Match<R>(Func<int, R> success, Func<int, string, R> error)
        {
            if (success == null)
            {
                throw new ArgumentNullException(nameof(success));
    ...
    public struct ApiResponse<T> : IEquatable<ApiResponse<T>>
    {
        /// <summary>
        /// The status code of the response.
        /// </summary>
        ...
        /// <exception cref="ArgumentOutOfRangeException">The status code does not indicate success.</exception>
        public static ApiResponse<T> Success(T payload, int statusCode)
        {
            if (payload == null)
            {
        ...
        /// <summary>
        /// Creates a new instance of the <see cref="ApiResponse{T}"/> struct that represents a successful response with a status code of 200.
        /// </summary>
        /// <param name="payload">The payload of the response.</param>
        /// <returns>A new instance of the <see cref="ApiResponse{T}"/> struct that represents a successful response.</returns>
        ...
        /// <exception cref="ArgumentNullException">The payload is <c>null</c>.</exception>
        /// <exception cref="ArgumentOutOfRangeException">The status code does not indicate success.</exception>
        public static ApiResponse<T> Success(T payload, HttpStatusCode statusCode = HttpStatusCode.OK)
        {
            return Success(payload, HttpUtils.HttpStatusCodeToInt(statusCode));
        }
        ...
        public R Match<R>(Func<T, int, R> success, Func<int, string, R> error)
        {
            if (success == null)
            {
                throw new ArgumentNullException(nameof(success));
        ...
        /// </summary>
        /// <typeparam name="R">The type to convert the payload to.</typeparam>
        ...
        /// <exception cref="InvalidOperationException">The payload converter is <c>null</c>. Only if the Response if success and the converter is null.</exception>
        public ApiResponse<R> Into<R>(Func<T, R> payloadConverter)
        {
            ...
            return Match(
                ...
                {
                    return ApiResponse<R>.Success(payloadConverter(payload), statusCode);
                },
                error: (statusCode, errorMessage) => ApiResponse<R>.Error(statusCode, errorMessage)
            );
        }
